...
public:
    SvgPaintEnginePrivate()
    {
        ...
        attributes.startTempo = 0;
        attributes.startTicks = 0;
        attributes.endTicks = 0;
        attributes.startTime = 0.0;
        attributes.endTime = 0.0;
        attributes.timesigNum = 0;
        attributes.timesigDenom = 0;
...
#define SVG_TIME_BEGIN  "<time"
#define SVG_START_TICK  " starttick=\""
#define SVG_END_TICK    " endtick=\""
#define SVG_START_TIME  " starttime=\""
#define SVG_END_TIME    " endtime=\""
#define SVG_START_TEMPO " starttempo_bps=\""
#define SVG_TIMESIG_NUM  " timesig_num=\""
#define SVG_TIMESIG_DENOM  " timesig_denom=\""
...
public:
    ...
    qreal startTime() const { return d_func()->attributes.startTime; }
    void setStartTime(const qreal &startTime) {
        d_func()->attributes.startTime = startTime;
    }

    qreal endTime() const { return d_func()->attributes.endTime; }
    void setEndTime(const qreal &endTime) {
        d_func()->attributes.endTime = endTime;
    }

    int startTicks() const { return d_func()->attributes.startTicks; }
    void setStartTicks(const int &startTicks) {
        d_func()->attributes.startTicks = startTicks;
    }

    int endTicks() const { return d_func()->attributes.endTicks; }
    void setEndTicks(const int &endTicks) {
        d_func()->attributes.endTicks = endTicks;
    }

    qreal startTempo() const { return d_func()->attributes.startTempo; }
    void setStartTempo(const qreal &startTempo) {
        d_func()->attributes.startTempo = startTempo;
    }

    int timesigNum() const { return d_func()->attributes.timesigNum; }
    void setTimesigNum(const int &timesigNum) {
        d_func()->attributes.timesigNum = timesigNum;
    }

    int timesigDenom() const { return d_func()->attributes.timesigDenom; }
    void setTimesigDenom(const int &timesigDenom) {
        d_func()->attributes.timesigDenom = timesigDenom;
    }
...
int SvgGenerator::endTicks() const{
    Q_D(const SvgGenerator);
    return d->engine->endTicks();
}

void SvgGenerator::setEndTicks(const int &endTicks)
{
    Q_D(SvgGenerator);
    d->engine->setEndTicks(endTicks);
}

int SvgGenerator::startTicks() const{
    Q_D(const SvgGenerator);
    return d->engine->startTicks();
}

void SvgGenerator::setStartTicks(const int &startTicks)
{
    Q_D(SvgGenerator);
    d->engine->setStartTicks(startTicks);
}

qreal SvgGenerator::startTempo() const{
    Q_D(const SvgGenerator);
    return d->engine->startTempo();
}

void SvgGenerator::setStartTempo(const qreal &startTempo)
{
    Q_D(SvgGenerator);
    d->engine->setStartTempo(startTempo);
}

int SvgGenerator::timesigNum() const{
    Q_D(const SvgGenerator);
    return d->engine->timesigNum();
}

void SvgGenerator::setTimesigNum(const int &timesigNum)
{
    Q_D(SvgGenerator);
    d->engine->setTimesigNum(timesigNum);
}


int SvgGenerator::timesigDenom() const{
    Q_D(const SvgGenerator);
    return d->engine->timesigDenom();
}

void SvgGenerator::setTimesigDenom(const int &timesigDenom)
{
    Q_D(SvgGenerator);
    d->engine->setTimesigDenom(timesigDenom);
}


qreal SvgGenerator::endTime() const{
    Q_D(const SvgGenerator);
    return d->engine->endTime();
}

void SvgGenerator::setEndTime(const qreal &endTime)
{
    Q_D(SvgGenerator);
    d->engine->setEndTime(endTime);
}

qreal SvgGenerator::startTime() const{
    Q_D(const SvgGenerator);
    return d->engine->startTime();
}

void SvgGenerator::setStartTime(const qreal &startTime)
{
    Q_D(SvgGenerator);
    d->engine->setStartTime(startTime);
}
...
bool SvgPaintEngine::begin(QPaintDevice *)
{
    ...
    stream() << SVG_TIME_BEGIN;
    stream() << SVG_START_TEMPO << d->attributes.startTempo << SVG_QUOTE;
    stream() << SVG_TIMESIG_NUM << d->attributes.timesigNum << SVG_QUOTE;
    stream() << SVG_TIMESIG_DENOM << d->attributes.timesigDenom << SVG_QUOTE;
    stream() << SVG_START_TICK << d->attributes.startTicks << SVG_QUOTE;
    stream() << SVG_END_TICK << d->attributes.endTicks << SVG_QUOTE;
    stream() << SVG_START_TIME << d->attributes.startTime << SVG_QUOTE;
    stream() << SVG_END_TIME << d->attributes.endTime << SVG_QUOTE << SVG_ELEMENT_END << endl;
